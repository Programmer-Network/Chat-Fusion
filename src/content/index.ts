interface ISelectorConfig {
    ...
    chatMessageLinkSelector: string;
    ...
    try {
        return {
            "www.twitch.tv": {
                ...
                chatMessageLinkSelector: ".chat-line__message .link-fragment",
            ...
            "www.youtube.com": {
                ...
                chatMessageAuthorBadgeSelector:
                    ".yt-live-chat-app #author-photo",
                chatMessageContentSelector:
                    ".yt-live-chat-text-message-renderer #message",
                chatMessageLinkSelector:
                    ".yt-live-chat-text-message-renderer #message a",
            ...
            Array.from(mutation.addedNodes).forEach((chatNode) => {
                ...
                const content =
                    config.chatMessageContentSelector && isHTMLElement
                        ? chatNode.querySelector(
                              config.chatMessageContentSelector
                          )?.textContent
                        : "";
                ...
                const author =
                    config.chatMessageAuthorSelector && isHTMLElement
                        ? chatNode.querySelector(
                              config.chatMessageAuthorSelector
                          )?.textContent
                        : "";
                ...
                const link =
                    platform === "twitch" &&
                    config.chatMessageLinkSelector &&
                    isHTMLElement
                        ? chatNode.querySelector(config.chatMessageLinkSelector)
                              ?.textContent
                        : "";
                ...
                // user can send only link
                if (link && author) {
                    fetch("http://localhost:3000/api/save-messages/links", {
                        method: "POST",
                        headers: {
